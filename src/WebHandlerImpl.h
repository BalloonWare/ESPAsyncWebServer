...
public:
  AsyncStaticWebHandler(const char *uri, FS &fs, const char *path,
  ...
  setLastModified(); // sets to current time. Make sure sntp is runing and time
                     // is updated
#endif
  AsyncStaticWebHandler &
  setTemplateProcessor(AwsTemplateProcessor newCallback) {
...
public:
  AsyncCallbackWebHandler()
      : _uri(), _method(HTTP_ANY), _onRequest(NULL), _onUpload(NULL),
        _onBody(NULL), _isRegex(false) {}
  ...
  virtual bool canHandle(AsyncWebServerRequest *request) override final {
    ...
    if (_isRegex) {
      ...
      if (!request->url().startsWith(uriTemplate))
        return false;
    } else if (_uri.length() && (_uri != request->url() &&
